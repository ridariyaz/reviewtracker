...
<body>
  ...
  <div id="spotlightTourOverlay" style="display:none; position:fixed; inset:0; z-index:9999; pointer-events:auto;">
    <div id="spotlightHighlightBox" style="display:none; position:fixed; z-index:10000; border:2.5px solid var(--primary); border-radius:14px; box-shadow:0 0 0 9999px rgba(15, 23, 42, 0.78), 0 0 25px rgba(37, 99, 235, 0.6); pointer-events:none; transition:all 0.25s ease;"></div>
    <div id="spotlightTooltipCard" style="position:fixed; z-index:10002; background:var(--card-bg); color:var(--text-main); border:1px solid var(--border-color); width:320px; border-radius:16px; padding:20px; box-shadow:0 25px 60px rgba(0,0,0,0.75); transition:all 0.3s ease;">
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      @if(session('trigger_tour'))
        localStorage.removeItem('reviewtracker_tour_done');
        setTimeout(function () {
          startSpotlightTour();
        }, 600);
      @else
